...
import com.flyimg.service.impl.UploadServicel;
...
public class UpdateImgController {
    ...
    public String indexImg(Model model, HttpSession httpSession, HttpServletRequest request, HttpServletResponse response) {
            ...
            if(sysConfig.getMaxSizeTourists()!=null){filesizetourists = sysConfig.getMaxSizeTourists();}
            if(sysConfig.getMaxSizeUser()!=null){filesizeuser = sysConfig.getMaxSizeUser();}
            if(sysConfig.getMaxCountTourists()!=null){imgcounttourists = sysConfig.getMaxCountTourists();}
            if(sysConfig.getMaxCountUser()!=null){imgcountuser = sysConfig.getMaxCountUser();}
            if (email != null) {
            ...
            model.addAttribute("suffix", sysConfig.getSuffix());
            model.addAttribute("config", config);
            model.addAttribute("uploadConfig", sysConfig);
            Integer isupdate = 1;
            if(sysConfig.getAllowTouristsUp()!=1){
                isupdate = (u == null) ? 0: 1;
            }
...
//根据网络图片url上传
    ...
    public String upurlimg(HttpSession session, String imgurl, HttpServletRequest request,Integer setday,String upurlk) throws Exception {
        ...
        iparr = sysConfig.getBlacklist().split(";");
        for (String s : iparr) {
            if(s.equals(userip)){
                jsonArray.add(911);
        ...
        if(u==null){
            ...
            memory = sysConfig.getMaxMemoryTourists();
            usermemory= fileService.getusermemory(0);
            if(usermemory==null){usermemory = 0;}
        }else{
            userpath = u.getUsername();
            Sourcekey = GetCurrentSource.GetSource(u.getId());
            memory = userService.get(u.getEmail()).getMemory();
            usermemory= fileService.getusermemory(u.getId());
            if(usermemory==null){usermemory = 0;}
        }
        if(sysConfig.getUrltype()==2){
            java.text.DateFormat dateFormat = new SimpleDateFormat("yyyy/MM/dd");
            userpath = dateFormat.format(new Date());
        }
        if(Integer.parseInt(CryptoUtils.encodeBASE64(upurlk))!=yzupdate()){
            jsonArray.add(-403);
            return jsonArray.toString();
        }
        Keys key = keysService.selectKeys(Sourcekey);
        long imgsize = ImgUtils.getFileLength(imgurl);
        Integer youke = sysConfig.getMaxSizeTourists();
        Integer yonghu = sysConfig.getMaxSizeUser();
        String uuid= UUID.randomUUID().toString().replace("-", "");
        Boolean bo =false;
        bo = Sourcekey==5?true: StringUtilsMy.doNull(Sourcekey,key);
        if(!bo){
            jsonArray.add(-1);
            return jsonArray.toString();
        }
        ...
        if(u!=null){
            ...
            try{
                boolean bl = ImgUtils.downLoadFromUrl(imgurl,
                ...
                if(bl==true){
                    ...
                    String xxx = ImgUtils.bytesToHexString(b);
                    xxx = xxx.toUpperCase();
                    if(is!=null){is.close();}
                    if(ImgUtils.checkPictureType(xxx).equals("0000")) {
                        jsonArray.add(-3);//不是图片格式
                        return jsonArray.toString();
                    }
                    Map<String, String> map = new HashMap<>();
                    map.put(ImgUtils.checkPictureType(xxx), request.getSession().getServletContext().getRealPath("/")+"hellohaotmp"+ File.separator+uuid);
                    Map<ReturnImage, Integer> m = null;
                    m = GetSource.storageSource(key.getStorageType(), null, userpath,map,setday);
                    FileOSS img = new FileOSS();
                    ...
                    for (Map.Entry<ReturnImage, Integer> entry : m.entrySet()) {
                        if(key.getStorageType()==5){
                            if(config.getDomain()!=null){
                                ...
                                img.setUri(config.getDomain()+"/"+entry.getKey().getImgurl());//图片链接
                            }else{
                                jsonArray.add(config.getDomain()+"/"+entry.getKey().getImgurl());
                                img.setUri("http://"+ IpUtils.getLocalIP()+":"+ IpUtils.getLocalPort()+"/"+entry.getKey().getImgurl());//图片链接
                            }
                        }else{
                            jsonArray.add(entry.getKey().getImgurl());
                            img.setUri(entry.getKey().getImgurl());
                        }
                        ...
                        img.setFilename(entry.getKey().getImgurl());
            ...
            try{
                ...
                if(bl==true){
                    ...
                    Map<String, String> map = new HashMap<>();
                    map.put(ImgUtils.checkPictureType(xxx), request.getSession().getServletContext().getRealPath("/")+File.separator+"hellohaotmp"+File.separator+uuid);
                    Map<ReturnImage, Integer> m = null;
                    m = GetSource.storageSource(key.getStorageType(), null, userpath,map,setday);
                    FileOSS img = new FileOSS();
                    SimpleDateFormat df=new SimpleDateFormat("yyyy-MM-dd");
                    ...
                    for (Map.Entry<ReturnImage, Integer> entry : m.entrySet()) {
                        if(key.getStorageType()==5){
                            if(config.getDomain()!=null){
                                jsonArray.add(config.getDomain()+"/"+entry.getKey().getImgurl());
                                img.setUri(config.getDomain()+"/"+entry.getKey().getImgurl());
                            }else{
                                jsonArray.add(config.getDomain()+"/"+entry.getKey().getImgurl());
                                img.setUri("http://"+ IpUtils.getLocalIP()+":"+ IpUtils.getLocalPort()+"/"+entry.getKey().getImgurl());//图片链接
                            }
                        ...
                        img.setUpdatetime(times);
                        img.setUserid(u == null?0:u.getId());
                        img.setFilename(SetText.getSubString(entry.getKey().getImgurl(), key.getRequestAddress() + "/", ""));
